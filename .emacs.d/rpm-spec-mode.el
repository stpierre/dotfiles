...
(defcustom rpm-spec-rpmlint-options "-i"
  "*Command line options to be used with rpmlint call"
  :type 'string
  :group 'rpm-spec)
...
(unless rpm-spec-mode-map
  ...
  (define-key rpm-spec-mode-map "\C-c\C-k"  'rpm-rpmlint)
...
(defun rpm-rpmlint ()
  "Run rpmlint against the file behind the current buffer after
   checking if unsaved buffers should be saved."
  (require 'compile)
  
  (interactive)
  (let* ((file (buffer-file-name (current-buffer)))
         (command (concat "rpmlint " rpm-spec-rpmlint-options
                          " \"" file "\"")))
